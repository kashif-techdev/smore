...
from smore.losses import CombinedLoss, combined_loss  # noqa: E402
...
def test_combined_loss_prefers_matching_inputs() -> None:
    x = torch.rand(2, 1, 32, 32)
    same = combined_loss(x, x).item()
    diff = combined_loss(x, torch.rand_like(x)).item()
    assert same < diff


def test_combined_loss_backward() -> None:
    model = torch.nn.Conv2d(1, 1, 3, padding=1)
    criterion = CombinedLoss(ssim_alpha=0.8)
    x = torch.rand(2, 1, 32, 32)
...
def main() -> None:
    ...
    test_combined_loss_prefers_matching_inputs()
